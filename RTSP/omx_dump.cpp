...
const char* dump_OMX_COLOR_FORMATTYPE(OMX_COLOR_FORMATTYPE color) {
  switch (color) {
    ...
    default:
      return "unknown OMX_COLOR_FORMATTYPE";
  }
}
...
const char* dump_OMX_OTHER_FORMATTYPE(OMX_OTHER_FORMATTYPE format) {
  switch (format) {
    ...
    DUMP_CASE(OMX_OTHER_FormatText3GP5)
      ;
    default:
      return "unknown OMX_OTHER_FORMATTYPE";
  }
}
...
const char* dump_OMX_AUDIO_CODINGTYPE(OMX_AUDIO_CODINGTYPE coding) {
  switch (coding) {
    ...
    default:
      return "unknown OMX_AUDIO_CODINGTYPE";
  }
}
...
const char* dump_OMX_VIDEO_CODINGTYPE(OMX_VIDEO_CODINGTYPE coding) {
  switch (coding) {
    ...
    default:
      return "unknown OMX_VIDEO_CODINGTYPE";
  }
}
...
const char* dump_OMX_IMAGE_CODINGTYPE(OMX_IMAGE_CODINGTYPE coding) {
  switch (coding) {
    ...
    default:
      return "unknown OMX_IMAGE_CODINGTYPE";
  }
}
...
void dump_OMX_PARAM_PORTDEFINITIONTYPE(OMX_PARAM_PORTDEFINITIONTYPE* port) {
  ...
  switch (port->eDomain) {
    case OMX_PortDomainAudio:
      ...
      sprintf(domain_info, "    cMIMEType: %s\n"
    ...
    case OMX_PortDomainVideo:
      ...
      sprintf(domain_info, "    cMIMEType: %s\n"
              ...
              "    eColorFormat: %s\n",
              port->format.video.cMIMEType, port->format.video.nFrameWidth,
              port->format.video.nFrameHeight, port->format.video.nStride,
              port->format.video.nSliceHeight, port->format.video.nBitrate,
    ...
    case OMX_PortDomainImage:
      ...
      sprintf(domain_info, "    cMIMEType: %s\n"
              ...
              "    eCompressionFormat: %s\n"
              "    eColorFormat: %s\n",
              port->format.image.cMIMEType, port->format.image.nFrameWidth,
              port->format.image.nFrameHeight, port->format.image.nStride,
  ...
  printf("nSize: %d\n"
         ...
         "nBufferAlignment: %d\n",
         port->nSize, port->nPortIndex,
         port->eDir == OMX_DirInput ? "input" : "output",
         port->nBufferCountActual, port->nBufferCountMin, port->nBufferSize,
         port->bEnabled ? "true" : "false", port->bPopulated ? "true" : "false",
         domain, domain_info, port->bBuffersContiguous ? "true" : "false",
...
void dump_OMX_IMAGE_PARAM_PORTFORMATTYPE(OMX_IMAGE_PARAM_PORTFORMATTYPE* port) {
  printf("nSize: %d\n"
         ...
         "eCompressionFormat: %s\n"
         "eColorFormat: %s\n",
         port->nSize, port->nPortIndex, port->nIndex,
...
void dump_OMX_BUFFERHEADERTYPE(OMX_BUFFERHEADERTYPE* header) {
  ...
  printf("nSize: %d\n"
         ...
         "nOutputPortIndex: %d\n"
         "nInputPortIndex: %d\n",
         header->nSize, header->nAllocLen, header->nFilledLen, header->nOffset,
         header->hMarkTargetComponent ? "not null" : "null (no mark)",
         header->nTickCount, timestamp, header->nFlags,
         header->nOutputPortIndex, header->nInputPortIndex);
}
